...
import statuses from "@src/__mocks__/database/status";
...
describe("GET /campaigns/{cid}/statuses", () => {
  beforeAll(async () => {
    await dbAdapter.add({
  ...
  it("Should answer 403 if user is not logged in", async () => {
    const response = await request(app).get(
      `/campaigns/${campaign_1.id}/statuses`
    );
  ...
  it("Should return 200 if the user is the owner", async () => {
    const response = await request(app)
      .get(`/campaigns/${campaign_1.id}/statuses`)
      .set("Authorization", "Bearer user");
  ...
  it("Should fail if the user is not the owner", async () => {
    const response = await request(app)
      .get(`/campaigns/${campaign_2.id}/statuses`)
      .set("Authorization", "Bearer user");
    expect(response.status).toBe(403);
  });

  // It should return the list of statuses
  it("Should return the list of statuses ordered by id DESC", async () => {
    const response = await request(app)
      .get(`/campaigns/${campaign_1.id}/statuses`)
      .set("Authorization", "Bearer user");
    expect(response.status).toBe(200);
    ...
    expect(body).toEqual(statuses.getDefaultItems().sort((t1, t2) => t2.id - t1.id));
  });
