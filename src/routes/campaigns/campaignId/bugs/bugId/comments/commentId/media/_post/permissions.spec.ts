...
describe("POST /campaigns/{cid}/bugs/{bid}/comments/{cmid}/media", () => {
  beforeAll(async () => {
  ...
  it("Should answer 403 if user is not logged in", async () => {
    const response = await request(app)
      .post(
        `/campaigns/${campaign_1.id}/bugs/${bug_1.id}/comments/${comment_1.id}/media`
      )
      .attach("media", Buffer.from("some data"), "image.png");
  ...
  it("Should answer 400 if campaign does not exist", async () => {
    const response = await request(app)
      .post(`/campaigns/99999/bugs/${bug_1.id}/comments/${comment_1.id}/media`)
  ...
  it("Should answer 400 if bug does not exist", async () => {
    const response = await request(app)
      .post(
        `/campaigns/${campaign_1.id}/bugs/8512/comments/${comment_1.id}/media`
      )
      .attach("media", Buffer.from("some data"), "image.png")
      .set("Authorization", "Bearer user");
    expect(response.status).toBe(400);
  ...
  it("Should answer 400 if the comment does not exist", async () => {
    const response = await request(app)
      .post(`/campaigns/${campaign_1.id}/bugs/${bug_1.id}/comments/99999/media`)
      .attach("media", Buffer.from("some data"), "image.png")
      .set("Authorization", "Bearer user");
    expect(response.status).toBe(400);
  });
  ...
  it("It should answer 403 if the campaign exists but the user has no permissions to see the campaign", async () => {
    const response = await request(app)
      .post(
        `/campaigns/${campaign_2.id}/bugs/${bug_2.id}/comments/${comment_2.id}/media`
      )
      .attach("media", Buffer.from("some data"), "image.png")
      .set("Authorization", "Bearer user");
